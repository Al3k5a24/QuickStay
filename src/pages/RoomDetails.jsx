    ...
    <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        ...
        <div className='flex flex-col md:flex-row items-start md:items-center gap-2'>
            <h1 className='text-3x1 md:text-4xl font-playfair'> {room.hotel.name}
             <span className='ml-2 font-inter text-sm'>({room.roomType})</span></h1>
            <p className='text-xs font-inter py-1.5 px-3
             text-white bg-[#2f68ed] rounded-full'>20% OFF</p>
        </div>
        ...
        <div className='flex flex-col lg:flex-row mt-6 gap-6'>
            ...
            <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
                {room?.images.length>1 && room.images.map((image,index)=>(
                    ...
                    object-cover cursor-pointer ${mainImage===image && 'outline outline-orange-500'}`}/>
                ))}
            </div>
        </div>
        ...
        {/* Common Specification */}
            ...
            hover:bg-primary-dull active:scale-95 transition-all cursor-pointer'>
                Contact now
            </button>
    </div>
